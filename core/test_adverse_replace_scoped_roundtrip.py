...
if __name__ == "__main__":
    ...
    action_id_a = enqueue_test_action(
        created_by="test_adverse_replace_scope_A",
        limit_price=80500.0,
        note="Scoped adverse-replace target action",
    )
    action_id_b = enqueue_test_action(
        created_by="test_adverse_replace_scope_B",
        limit_price=80500.0,
        note="Scoped adverse-replace untouched action",
    )
    ...
    # Малък pause, за да има по-стабилни timestamps в логовете
    time.sleep(1.0)
    ...
    reprice_cfg = RepriceWorkerConfig(
        ...
        symbols=(symbol,),
        created_by_prefixes=("test_adverse_replace_scope_",),
    ...
    # adverse_up за OPEN+SHORT:
    # current = 80500, target = 80600
    # drift ≈ 12.42 bps > adverse_threshold_bps=10.0
    target_price_for_a = 80600.0
    ...
    print("\n=== run_ops_pass (SCOPED ADVERSE-REPLACE) ===")
    ...
    if reprice_candidates > 2:
        raise AssertionError(
            f"Scoped reprice pass saw too many candidates: {reprice_candidates}"
        )

    if reconcile_candidates > 2:
        raise AssertionError(
            f"Scoped reconcile pass saw too many candidates: {reconcile_candidates}"
        )

    reprice_result_a = ops_result["reprice_result"]["results"][0]
    reprice_result_b = ops_result["reprice_result"]["results"][1]
    ...
    assert float(row_a_after["limit_price"]) == 80600.0
    ...
    assert is_order_open(symbol, new_order_id_a) is True
    assert is_order_open(symbol, old_order_id_b) is True

    assert reprice_result_a["stage"] == "replaced"
    assert reprice_result_a["applied"] is True
    assert reprice_result_a["policy_decision"]["overshoot_kind"] == "adverse_up"
    assert float(reprice_result_a["policy_decision"]["effective_threshold_bps"]) == 10.0
    assert float(reprice_result_a["policy_decision"]["target_price"]) == 80600.0

    assert reprice_result_b["stage"] == "skip"
    assert reprice_result_b["reason"] == "missing_target_price"

    print("SCOPED ADVERSE-REPLACE OPS LOOP TEST PASSED")