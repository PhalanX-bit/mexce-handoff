...
if __name__ == "__main__":
    ...
    action_id_a = enqueue_test_action(
        created_by="test_adverse_scope_A",
        limit_price=80500.0,
        note="Scoped adverse-threshold target action",
    )
    action_id_b = enqueue_test_action(
        created_by="test_adverse_scope_B",
        limit_price=80500.0,
        note="Scoped adverse-threshold untouched action",
    )
    ...
    reprice_cfg = RepriceWorkerConfig(
        ...
        symbols=(symbol,),
        created_by_prefixes=("test_adverse_scope_",),
    ...
    # adverse_up for OPEN+SHORT:
    # current=80500, target=80540 => drift ~4.97 bps
    # Това е над base threshold (2.0), но под adverse threshold (10.0)
    # Значи НЕ трябва да има replace, ако adverse_threshold_bps се спазва правилно.
    target_price_for_a = 80540.0
    ...
    print("\n=== run_ops_pass (SCOPED ADVERSE-THRESHOLD) ===")
    ...
    if reprice_candidates > 2:
        raise AssertionError(
            f"Scoped adverse-threshold reprice pass saw too many candidates: {reprice_candidates}"
        )

    if reconcile_candidates > 2:
        raise AssertionError(
            f"Scoped adverse-threshold reconcile pass saw too many candidates: {reconcile_candidates}"
        )
    ...
    assert result_a["overshoot_kind"] == "adverse_up"
    assert result_a["effective_threshold_bps"] == 10.0
    assert result_a["should_replace"] is False
    ...
    print("SCOPED ADVERSE-THRESHOLD OPS LOOP TEST PASSED")