...
if __name__ == "__main__":
    ...
    action_id_a = enqueue_test_action(
        created_by="test_not_open_scope_A",
        limit_price=80500.0,
        note="Scoped not-open target action",
    )
    action_id_b = enqueue_test_action(
        created_by="test_not_open_scope_B",
        limit_price=80500.0,
        note="Scoped not-open untouched action",
    )
    ...
    print(f"old_order_a_open_before_cancel = {is_order_open(symbol, old_order_id_a)}")
    print(f"old_order_b_open_before_cancel = {is_order_open(symbol, old_order_id_b)}")

    print("\n=== cancel order A manually ===")
    cancel_a = cancel_order_raw(old_order_id_a)
    pprint(cancel_a)

    print("\n=== open state after manual cancel ===")
    print(f"old_order_a_open_after_cancel = {is_order_open(symbol, old_order_id_a)}")
    print(f"old_order_b_open_after_cancel = {is_order_open(symbol, old_order_id_b)}")
    ...
    reprice_cfg = RepriceWorkerConfig(
        ...
        symbols=(symbol,),
        created_by_prefixes=("test_not_open_scope_",),
    ...
    print("\n=== run_ops_pass (SCOPED ORDER-NOT-OPEN) ===")
    ...
    if reprice_candidates != 2:
        raise AssertionError(
            f"Scoped reprice pass should see exactly 2 candidates, got: {reprice_candidates}"
        )

    if reconcile_candidates != 2:
        raise AssertionError(
            f"Scoped reconcile pass should see exactly 2 candidates, got: {reconcile_candidates}"
        )

    reprice_results = ops_result["reprice_result"]["results"]
    reprice_a = next(x for x in reprice_results if int(x["action_id"]) == action_id_a)
    reprice_b = next(x for x in reprice_results if int(x["action_id"]) == action_id_b)
    ...
    print("\n=== open order item A ===")
    pprint(find_open_order(symbol, current_order_id_a))
    ...
    assert current_order_id_a == old_order_id_a
    assert current_order_id_b == old_order_id_b

    assert float(row_a_after["limit_price"]) == 80500.0
    assert float(row_b_after["limit_price"]) == 80500.0

    assert reprice_a["stage"] == "noop"
    assert reprice_a["applied"] is False
    assert reprice_a["should_replace"] is False
    assert reprice_a["decision_reason"] == "order_not_open"
    assert reprice_a["order_open"] is False

    assert reprice_b["stage"] == "skip"
    assert reprice_b["reason"] == "missing_target_price"

    assert is_order_open(symbol, old_order_id_a) is False
    assert is_order_open(symbol, old_order_id_b) is True

    # При текущата reconcile логика след ръчен cancel очакваме action A да не е OPEN
    assert row_a_after["reconcile_state"] != "OPEN"
    assert row_a_after["reconcile_state"] in ("NOT_FOUND", "CANCELED", "FILLED", "PARTIAL", None)

    assert row_b_after["reconcile_state"] == "OPEN"

    print("SCOPED ORDER-NOT-OPEN OPS LOOP TEST PASSED")