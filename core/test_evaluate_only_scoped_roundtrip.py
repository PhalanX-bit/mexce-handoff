...
if __name__ == "__main__":
    ...
    action_id_a = enqueue_test_action(
        created_by="test_eval_only_scope_A",
        limit_price=80500.0,
        note="Scoped evaluate-only target action",
    )
    action_id_b = enqueue_test_action(
        created_by="test_eval_only_scope_B",
        limit_price=80500.0,
        note="Scoped evaluate-only untouched action",
    )
    ...
    reprice_cfg = RepriceWorkerConfig(
        ...
        symbols=(symbol,),
        created_by_prefixes=("test_eval_only_scope_",),
    ...
    print("\n=== run_ops_pass (SCOPED EVALUATE-ONLY) ===")
    ...
    print("\n=== open state checks ===")
    print(f"order_a_open_after = {is_order_open(symbol, current_order_id_a)}")
    print(f"order_b_open_after = {is_order_open(symbol, current_order_id_b)}")
    ...
    if reprice_candidates > 2:
        raise AssertionError(
            f"Scoped evaluate-only reprice pass saw too many candidates: {reprice_candidates}"
        )

    if reconcile_candidates > 2:
        raise AssertionError(
            f"Scoped evaluate-only reconcile pass saw too many candidates: {reconcile_candidates}"
        )

    reprice_results = ops_result["reprice_result"]["results"]
    reprice_result_a = next(
        (x for x in reprice_results if int(x.get("action_id")) == action_id_a),
        None,
    )
    reprice_result_b = next(
        (x for x in reprice_results if int(x.get("action_id")) == action_id_b),
        None,
    )

    print("\n=== reprice result A ===")
    pprint(reprice_result_a)

    print("\n=== reprice result B ===")
    pprint(reprice_result_b)
    ...
    assert row_a_after["api_mode"] == "direct_futures_api"
    assert row_b_after["api_mode"] == "direct_futures_api"

    assert row_a_after["reconcile_state"] == "OPEN"
    assert row_b_after["reconcile_state"] == "OPEN"

    assert reprice_result_a is not None
    assert reprice_result_a["ok"] is True
    assert reprice_result_a["applied"] is False
    assert reprice_result_a["stage"] == "would_replace"
    assert reprice_result_a["should_replace"] is True
    assert reprice_result_a["decision_reason"] == "stale_by_drift"
    assert str(reprice_result_a["api_order_id"]) == old_order_id_a

    assert reprice_result_b is not None
    assert reprice_result_b["stage"] == "skip"
    assert reprice_result_b["reason"] == "missing_target_price"

    assert is_order_open(symbol, current_order_id_a) is True
    assert is_order_open(symbol, current_order_id_b) is True

    print("SCOPED EVALUATE-ONLY OPS LOOP TEST PASSED")