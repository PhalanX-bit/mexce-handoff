...
if __name__ == "__main__":
    ...
    action_id_a = enqueue_test_action(
        created_by="test_stale_age_scope_A",
        limit_price=80500.0,
        note="Scoped stale-by-age target action",
    )
    action_id_b = enqueue_test_action(
        created_by="test_stale_age_scope_B",
        limit_price=80500.0,
        note="Scoped stale-by-age untouched action",
    )
    ...
    target_price_for_a = 80500.0

    print("\n=== aging orders intentionally ===")
    time.sleep(3.0)
    ...
    reprice_cfg = RepriceWorkerConfig(
        ...
        symbols=(symbol,),
        created_by_prefixes=("test_stale_age_scope_",),
    ...
    print("\n=== run_ops_pass (SCOPED STALE-BY-AGE) ===")
    ...
    if reprice_candidates > 2:
        raise AssertionError(
            f"Scoped stale-by-age reprice pass saw too many candidates: {reprice_candidates}"
        )

    if reconcile_candidates > 2:
        raise AssertionError(
            f"Scoped stale-by-age reconcile pass saw too many candidates: {reconcile_candidates}"
        )
    ...
    assert reprice_result_a["policy_decision"]["should_replace"] is True
    assert reprice_result_a["policy_decision"]["decision_reason"] == "stale_by_age"
    assert reprice_result_a["policy_decision"]["stale_by_age"] is True
    assert reprice_result_a["policy_decision"]["stale_by_drift"] is False

    assert reprice_result_b["stage"] == "skip"
    assert reprice_result_b["reason"] == "missing_target_price"

    print("SCOPED STALE-BY-AGE OPS LOOP TEST PASSED")