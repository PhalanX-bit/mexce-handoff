...
if __name__ == "__main__":
    ...
    # action A -> ще бъде repriced
    action_id_a = enqueue_test_action(
        created_by="test_ops_loop_roundtrip_A",
        limit_price=80500.0,
        note="Ops loop target action",
    )

    # action B -> ще остане без target и ще бъде skip-нат от reprice pass
    action_id_b = enqueue_test_action(
        created_by="test_ops_loop_roundtrip_B",
        limit_price=80500.0,
        note="Ops loop untouched action",
    )
