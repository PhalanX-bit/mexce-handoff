...
if __name__ == "__main__":
    ...
    # Важното: scope-ът да вижда само action-ите от ТОВА изпълнение на теста
    test_started_at = utc_now_iso()
    print(f"test_started_at = {test_started_at}")

    # Двата нови scoped action-а
    action_id_a = enqueue_test_action(
        created_by="test_ops_scope_A",
        limit_price=80500.0,
        note="Scoped ops target action",
    )
    action_id_b = enqueue_test_action(
        created_by="test_ops_scope_B",
        limit_price=80500.0,
        note="Scoped ops untouched action",
    )
    ...
    reprice_cfg = RepriceWorkerConfig(
        ...
        symbols=(symbol,),
        created_by_prefixes=("test_ops_scope_",),
    ...
    assert float(row_a_after["limit_price"]) == 80480.0
    ...
    print("SCOPED OPS LOOP TEST PASSED")