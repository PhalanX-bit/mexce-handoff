...
if __name__ == "__main__":
    ...
    action_id_a = enqueue_test_action(
        created_by="test_noop_scope_A",
        limit_price=80500.0,
        note="Scoped noop target action",
    )
    action_id_b = enqueue_test_action(
        created_by="test_noop_scope_B",
        limit_price=80500.0,
        note="Scoped noop untouched action",
    )
    ...
    reprice_cfg = RepriceWorkerConfig(
        ...
        symbols=(symbol,),
        created_by_prefixes=("test_noop_scope_",),
    ...
    # За OPEN SHORT favorable move е target_price < current_price.
    # Тук даваме много малък favorable drift:
    # 80500 -> 80495 = ~0.621 bps, което е под favorable_threshold_bps=1.0
    target_price_for_a = 80495.0
    ...
    if reprice_candidates > 2:
        raise AssertionError(
            f"Scoped noop reprice pass saw too many candidates: {reprice_candidates}"
        )

    if reconcile_candidates > 2:
        raise AssertionError(
            f"Scoped noop reconcile pass saw too many candidates: {reconcile_candidates}"
        )

    print("\n=== reprice result A ===")
    pprint(ops_result["reprice_result"]["results"][0])
    ...
    # NOOP: id и price остават същите
    ...
    # reconcile пак трябва да види поръчките като OPEN
    assert row_a_after["reconcile_state"] == "OPEN"
    assert row_b_after["reconcile_state"] == "OPEN"

    # Проверка, че за A наистина е било noop
    result_a = None
    for item in ops_result["reprice_result"]["results"]:
        if int(item.get("action_id", -1)) == action_id_a:
            result_a = item
            break

    if result_a is None:
        raise AssertionError("Could not find result for action_id_a in reprice results")

    assert result_a["stage"] == "noop"
    assert result_a["applied"] is False
    assert result_a["decision_reason"] == "within_threshold_and_age"

    print("SCOPED NOOP OPS LOOP TEST PASSED")