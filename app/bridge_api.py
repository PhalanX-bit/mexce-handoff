...
def _parse_limit_deferred_note(note: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    # expected userscript note:
    # ... | DEFERRED_OK | symbol=... | side=... | price=... | qty=... | kind=LIMIT
    if "DEFERRED_OK" not in note_s:
        return None

    parts = [p.strip() for p in note_s.split("|")]
    out: Dict[str, Any] = {}
    kind = ""
    ...
    for p in parts:
        if p.startswith("symbol="):
            ...
            kind = str(p.split("=", 1)[1].strip()).upper()
...
def _get_latest_contracts(
    ...
    if not row:
        return 0.0
    return float(row["contracts"] or 0.0)


def _get_action_panel_mode(
    con: sqlite3.Connection,
    action_id: int,
) -> str:
    row = con.execute(
    ...
    return str(row["panel_mode"] or "OPEN").strip().upper()
...
def _upsert_pending_limit_task_from_done_note(
    ...
    panel_mode = _get_action_panel_mode(con, action_id)
    baseline_contracts = _get_latest_contracts(
        con,
        parsed["symbol"],
        parsed["side"],
    )
    ...
    con.execute(
        ...
        (action_id, symbol, side, panel_mode, limit_price, qty, status,
         trigger_seen, baseline_contracts, created_at, updated_at, note)
        ...
        ON CONFLICT(action_id) DO UPDATE SET
            ...
            qty=excluded.qty,
            baseline_contracts=excluded.baseline_contracts,
...
@app.get("/pending_chase_tasks")
def list_pending_chase_tasks(status: Optional[str] = None) -> Dict[str, Any]:
    con = connect()
    try:
        if status:
            rows = con.execute(
