...
# - Dashboard (latest account snapshot + latest positions per symbol+side) + OPEN/CLOSED status
# - Aggregates computed from OPEN legs only (so closed legs don't clutter)
...
def get_latest_positions_per_symbol_side(con):
    ...
    This works perfectly with the collector "reconcile" strategy:
