...
# Copy–paste ready (FULL FILE).
# Adds a 4th tab: Strategy (AUTO) that enqueues ARMED actions for the Tampermonkey executor.
#
# MVP strategy here is SAFE by design:
# - AUTO places ONLY OPEN + CHASE orders (qty-only)  ✅ matches your userscript
# - NO automatic CLOSE yet (because your userscript requires LIMIT price+qty for LIMIT)
# - Includes cooldown + guard: do not enqueue if ARMED/RUNNING exists for the symbol
#
# Run:
...
def get_latest_positions_per_symbol_side(con):
    """
    Returns the latest row per (symbol, side) by MAX(id).
    This works with reconcile:
    - if a leg is closed, collector inserts a latest row with contracts=0
    """
    rows = con.execute(
        """
...
def enqueue_armed_open_chase(con, *, symbol: str, side: str, qty: float, note: str, priority: int = 50) -> None:
    """
    Enqueue an ARMED task that the Tampermonkey executor can run immediately.
    This is SAFE with your userscript: OPEN + CHASE => qty-only fill.
    """
    p = {
        ...
        "qty": float(qty),
        "qty_unit": "contracts",
        "order_type": "market",     # executor treats CHASE as qty-only; order_type is informational
        "limit_price": None,
        "slippage_bps": None,
        ...
        "panel_mode": "OPEN",
        "order_kind": "CHASE",      # ✅ critical: matches executor's LIMIT/CHASE logic
        "leverage": None,
    }
...
def get_leg(df_pos: pd.DataFrame, symbol: str, side: str) -> dict:
    """
    Returns latest leg metrics for symbol+side from the 'latest per symbol+side' dataset.
    """
    if df_pos.empty:
...
tab1, tab2, tab3, tab4 = st.tabs(["Dashboard", "Action Queue", "Action Ledger", "Strategy (AUTO)"])
...
with tab1:
    ...
    if pos_rows:
        ...
        # Optionally hide closed legs
        if not show_closed_legs:
            df_pos_view = df_pos[df_pos["status"] == "OPEN"].copy()
        else:
            df_pos_view = df_pos.copy()

        # Signed contracts for exposure (CLOSED -> 0 anyway)
        ...
        # Aggregates
        if aggregates_open_only:
            df_for_agg = df_pos[df_pos["status"] == "OPEN"].copy()
        else:
            df_for_agg = df_pos.copy()
        ...
        with left:
            ...
            cols_show = ["symbol", "side", "status", "contracts", "entry_price", "unrealized_pnl", "created_at"]
...
with tab2:
    ...
    st.caption("Tip: Your executor supports only order_kind LIMIT/CHASE (MVP).")
    ...
    with st.form("queue_create_v2", clear_on_submit=True):
        ...
        order_kind = c5.selectbox(
            "Order kind",
            # Keep legacy options for manual use, but remember executor MVP runs LIMIT/CHASE only.
        ...
        if submit:
            ...
            p = {
                ...
                "order_kind": order_kind,  # manual can be anything; executor MVP runs LIMIT/CHASE
                "leverage": None,
            }
...
# =================== Strategy (AUTO) ===================
with tab4:
    st.subheader("Strategy (AUTO) — Hedge Cycle v1 (OPEN+CHASE only)")
    st.caption(
        "AUTO mode only enqueues OPEN+CHASE (qty-only) tasks for safety. "
        "No auto CLOSE yet (your executor requires LIMIT price+qty)."
    )
    ...
    c1, c2, c3, c4 = st.columns(4)
    sym = c1.text_input("Symbol", value="ADA/USDT").strip()
    initial_side = c2.selectbox("Initial side", ["LONG", "SHORT"], index=0)
    base_qty = c3.number_input("Base qty (contracts)", min_value=1.0, value=50.0, step=1.0)
    ...
    dry = d4.checkbox("Dry-run (no enqueue)", value=True)

    if autorefresh_available:
        r1, r2 = st.columns([1, 3])
        refresh_ms = r1.selectbox("Refresh interval", [2000, 3000, 5000, 8000], index=1)
        r2.caption("Auto-run uses autorefresh. Install if missing: pip install streamlit-autorefresh")
        if auto:
    ...
    # only evaluate on run_once OR in auto mode with reruns
    ...
    if do_eval:
        ...
        df_pos = pd.DataFrame(pos_rows) if pos_rows else pd.DataFrame(columns=["symbol", "side", "contracts", "entry_price", "unrealized_pnl"])
        if not df_pos.empty:
            df_pos["side"] = df_pos["side"].fillna("").astype(str).str.upper()
            _to_float_series(df_pos, "contracts")
            _to_float_series(df_pos, "entry_price")
        ...
        st.write("Latest snapshot:", {"account_ts": (acc or {}).get("created_at"), "positions_ts": pos_ts})
        st.write("Legs:", {"LONG": L, "SHORT": S})
        ...
        if cooldown_left > 0:
            ...
            st.info("Guard: ARMED/RUNNING task exists for this symbol. Waiting.")
            decision = None
        else:
            decision = None

            # MVP decision rules:
            # 1) no legs -> open initial side
            if L["contracts"] <= 0 and S["contracts"] <= 0:
                decision = ("OPEN", initial_side, base_qty, f"auto: initial {initial_side}")
            # 2) one-sided long -> hedge short if losing enough
            ...
            # 3) one-sided short -> hedge long if losing enough
            elif S["contracts"] > 0 and L["contracts"] <= 0:
                if S["unrealized_pnl"] <= float(hedge_loss_usdt):
                    decision = ("OPEN", "LONG", base_qty, "auto: hedge LONG")
            else:
                # both legs open -> MVP does nothing (no auto trim yet)
                decision = None

        st.write("Decision:", decision or "—")

        if decision:
            panel_mode, side, qty, note = decision[0], decision[1], decision[2], decision[3]
            if dry:
